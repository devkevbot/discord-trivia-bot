...
class TriviaSession {
  ...
  constructor(
    interaction,
    questions = [],
    timeToAnswerInSeconds = 20,
    questionDelayInSeconds = 10
  ) {
    this.#interaction = interaction;
    this.#questions = questions;
    this.#currentQuestionNum = 1;
    this.#leaderboard = new Leaderboard(interaction);
    this.#timeToAnswerInSeconds = timeToAnswerInSeconds;
    this.#questionDelayInSeconds = questionDelayInSeconds;
    this.#isActive = false;
  }
  ...
  async #collectUserAnswers() {
    const collected = await this.#interaction.channel.awaitMessages({
      filter: this.#filter,
      max: 1,
      time: this.#timeToAnswerInSeconds * 1000,
      errors: ['time'],
    });
    return collected;
  }
  ...
  async #onCorrectAnswer(author, answerTimeInSeconds) {
    this.#leaderboard.updatePlayerScore(author);
    await this.#postCorrectAnswerMessage(author, answerTimeInSeconds);
  }
  ...
  #wait(s) {
    return new Promise((resolve) => {
      this.#questionTimeoutID = setTimeout(resolve, s * 1000);
    });
  }
  ...
  async start() {
    ...
    while (this.#isActive && this.#hasNextQuestion()) {
      await this.#askQuestion();
      await this.#wait(this.#questionDelayInSeconds);
    }
  }

  /**
   * Ends the trivia session.
   */
  async end() {
    this.#isActive = false;
    ...
    await this.#leaderboard.postLeadboardMessage();
  }
}
